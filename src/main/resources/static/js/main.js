...
class HealthPlanetApp {
    ...
    async init() {
        this.loadUserData();
        await this.loadBadgesFromServer();
        this.initTheme();
    ...
    async loadBadgesFromServer() {
        try {
            ...
            // 使用正确的API端点获取徽章数据
            const response = await fetch('/api/badge/list', {
                headers: {
                ...
                if (result.code === 200) {
                    ...
                    this.badges = result.data.map((badge, index) => {
                        ...
                        return {
                            ...
                            icon: this.getBadgeIconClass(iconUrl),
                            iconUrl: iconUrl, // 保留原始iconUrl用于图片显示
    ...
    async displayBadges() {
        ...
        if (this.currentTheme === 'christmas') {
            // 圣诞主题：圣诞树挂徽章形式
            await this.displayBadgesChristmasTheme(container);
        } else {
            // 正常主题：网格布局形式
    ...
    async displayBadgesChristmasTheme(container) {
        // 创建圣诞树挂徽章的布局
        let christmasHTML = `
        ...
        // 从服务器获取圣诞限定徽章数据
        const christmasBadges = await this.getChristmasBadgesFromServer();
        
        // 为每个徽章创建悬挂的装饰
        christmasBadges.forEach((badge, index) => {
            // 计算徽章在树上的位置
    ...
    // 从服务器获取圣诞限定徽章数据
    async getChristmasBadgesFromServer() {
        try {
            const token = localStorage.getItem('accessToken') || sessionStorage.getItem('accessToken');
            if (!token) {
                // 如果没有token，返回默认的圣诞徽章
                return this.getDefaultChristmasBadges();
            }

            const response = await fetch('/api/badge/christmas-badges', {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
                ...
                if (result.code === 200) {
                    ...
                    return result.data.map(badge => {
                        ...
                        return {
                            ...
                            color: this.getBadgeColor(badge.level),
                            level: badge.level || 1,
                            position: this.getPositionForChristmasBadge(badge.badgeName), // 计算位置
                            earned: false, // 默认未获得，后续会根据用户数据更新
                            rarity: this.getRarityByLevel(badge.level)
                        };
                    });
                }
            ...
            console.error('获取圣诞限定徽章失败:', error);
        }
    ...
    // 获取默认的圣诞徽章数据
    getDefaultChristmasBadges() {
        const defaultChristmasBadges = [
            {
                id: 101,
                name: "圣诞老人",
        ...
        // 尝试将已获得的徽章状态同步到默认徽章数据中
        defaultChristmasBadges.forEach(defaultBadge => {
            const userBadge = this.badges.find(b => b.name === defaultBadge.name);
            if (userBadge) {
                defaultBadge.earned = userBadge.earned;
                defaultBadge.earnedDate = userBadge.earnedDate;
            }
        });
        
        return defaultChristmasBadges;
    }
    
    // 根据徽章名称计算在圣诞树上的位置
    getPositionForChristmasBadge(badgeName) {
        // 根据徽章的等级和名称来分配位置
        switch(badgeName) {
            case "圣诞之星":
                return 1; // 顶部位置
            ...
            return '<div class="col-6 col-md-4 col-lg-3 mb-4">' +
                '<div class="badge-card ' + (badge.earned ? 'owned' : 'locked') + ' badge-hover-effect" ' +
                    ...
                    '<div class="badge-icon mx-auto mb-3" style="background: ' + badge.color + ';">' +
                        (iconUrl ? 
                          '<img src="' + iconUrl + '" alt="' + badge.name + '" class="badge-img" onerror="this.style.display=\'none\'; this.parentElement.innerHTML=\'<i class=&quot;' + badge.icon + ' fa-2x&quot;></i>\';">' :
                          '<i class="' + badge.icon + ' fa-2x"></i>'
                        ) +
                    '</div>' +
    ...
    showBadgeDetails(badgeId) {
        ...
        const modalHtml = '<div class="modal fade" id="badgeDetailModal" tabindex="-1">' +
                ...
                '<div class="modal-content christmas-modal">' +
                    ...
                    '<div class="modal-body text-center">' +
                        '<div class="badge-icon-lg mx-auto mb-3" style="background: ' + badge.color + ';">' +
                            (iconUrl ? 
                              '<img src="' + iconUrl + '" alt="' + badge.name + '" style="width: 100%; height: 100%; object-fit: contain;" onerror="this.style.display=\'none\'; this.parentElement.innerHTML=\'<i class=&quot;' + badge.icon + ' fa-3x&quot; ></i>\';">' :
